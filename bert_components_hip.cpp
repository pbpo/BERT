...
void BertEmbeddings::backward(hipStream_t stream,
    ...
    // Dropout backward needs a non-const tensor, so we copy
    grad_after_dropout.copy_from_gpu(grad_output_embeddings, stream);
    dropout_.backward(stream, grad_after_dropout, cache.dropout_cache);
...
void BertLayer::backward(rocblas_handle blas_handle, hipStream_t stream,
    ...
    grad_ffn_dropout_output.allocate(grad_ffn_sum_input.dims_);
    grad_ffn_dropout_output.copy_from_gpu(grad_ffn_sum_input, stream);
    
    GpuTensor grad_attention_block_output;
    // FIXED: .dims to .dims_
    grad_attention_block_output.allocate(cache.attention_cache.self_attention_cache.context_layer.dims_);
    grad_attention_block_output.copy_from_gpu(grad_ffn_sum_input, stream);

    // --- 3. FFN 역전파 (Dropout -> Output Dense -> Intermediate Dense) ---
    GpuTensor grad_intermediate_output, grad_ffn_input;
    // FIXED: .dims to .dims_
    grad_intermediate_output.allocate(cache.ffn_intermediate_dense_cache.input->dims_);
    grad_ffn_input.allocate(cache.attention_cache.self_attention_cache.context_layer.dims_);

    // FIXED: Dropout::backward call with 3 arguments
    // The result is written in-place to grad_ffn_dropout_output
    ffn_output_dropout_.backward(stream, grad_ffn_dropout_output, cache.ffn_output_dropout_cache);
    ffn_output_dense_.backward(blas_handle, stream, grad_ffn_dropout_output, cache.ffn_output_dense_cache, grad_intermediate_output);
    ...
    // FIXED: .d_ptr to .d_ptr_
    launch_accumulate_kernel(stream, (float*)grad_attention_block_output.d_ptr_, (const float*)grad_ffn_input.d_ptr_, grad_attention_block_output.num_elements_);
...
void BertEncoder::forward(rocblas_handle blas_handle, hipStream_t stream,
    ...
    cache.layer_caches.resize(layers_.size());
